 ...
 */
static char *e1kU32toHex(uint32_t u32, uint32_t mask, char *buf)//###############buf的大小是多少##################
{
...
static void e1kHardReset(PE1KSTATE pThis)
{
...
#ifdef E1K_INIT_RA0
    memcpy(pThis->aRecAddr.au32, pThis->macConfigured.au8,//################au32和au8的大小分别是多少######################
    ...
    if (pThis->pDrvR3)
        pThis->pDrvR3->pfnSetPromiscuousMode(pThis->pDrvR3, false);//设置成非混杂模式
 ...
 * @thread  E1000_TX
 */
DECLINLINE(void) e1kPacketDump(PE1KSTATE pThis, const uint8_t *cpPacket, size_t cb, const char *pszText)//############没有检查包的大小#################
{
#ifdef DEBUG
...
DECLINLINE(void) e1kAdvanceRDH(PE1KSTATE pThis)
{
    ...
    if (++RDH * sizeof(E1KRXDESC) >= RDLEN)//##################乘法可以溢出吗##################
        RDH = 0;
    /*
...
DECLINLINE(unsigned) e1kRxDPrefetch(PE1KSTATE pThis)
{
    ...
    unsigned nDescsToFetch      = RT_MIN(nDescsAvailable, E1K_RXD_CACHE_SIZE - pThis->nRxDFetched);//最多只能缓存16个
 ...
 */
static int e1kRxChecksumOffload(PE1KSTATE pThis, const uint8_t *pFrame, size_t cb, E1KRXDST *pStatus)//###########未检查包的大小######################
{
    /** @todo
...
static int e1kHandleRxPacket(PE1KSTATE pThis, const void *pvBuf, size_t cb, E1KRXDST status)
{
    ...
    uint8_t   rxPacket[E1K_MAX_RX_PKT_SIZE];//16384
    ...
    if (cb < 60)//包的长度小于60
    {
        memset(rxPacket + cb, 0, 60 - cb);//填充0
        cb = 60;//因为包的最小长度是60,所以设置cb等于60
    }
    if (!(RCTL & RCTL_SECRC) && cb <= cbMax)//不需要计算strip crc,且cb小于cbMax
    {
        ...
        if (pThis->fEthernetCRC)
            *(uint32_t*)(rxPacket + cb) = RTCrc32(rxPacket, cb);//计算包的crc然后填充到rxPacket中###################好像可以栈溢出######################
        cb += sizeof(uint32_t);
        STAM_PROFILE_ADV_STOP(&pThis->StatReceiveCRC, a);
        E1kLog3(("%s Added FCS (cb=%u)\n", pThis->szPrf, cb));
    }
    /* Compute checksum of complete packet */
    uint16_t checksum = e1kCSum16(rxPacket + GET_BITS(RXCSUM, PCSS), cb);//########是否可以越界读########
    e1kRxChecksumOffload(pThis, rxPacket, cb, &status);

    /* Update stats */
    E1K_INC_CNT32(GPRC);
    if (e1kIsBroadcast(pvBuf))//是广播包
        E1K_INC_CNT32(BPRC);
    else if (e1kIsMulticast(pvBuf))//是多播包
        E1K_INC_CNT32(MPRC);
...
# ifdef E1K_WITH_RXD_CACHE
    while (cb > 0)//cb大于0
    {
        E1KRXDESC *pDesc = e1kRxDGet(pThis);//获取一个E1KRXDESC结构体指针

        if (pDesc == NULL)//是空指针,说明receiver buffers不够
        {
...
# else /* !E1K_WITH_RXD_CACHE */
    if (RDH == RDT)//RDH等于RDT,说明receiver buffers是满的,打印log
    {
        E1kLog(("%s Out of receive buffers, dropping the packet\n",
                pThis->szPrf));
    }
    /* Store the packet to receive buffers */
    while (RDH != RDT)//RDH不等于RDT,可以收包
    {
        /* Load the descriptor pointed by head */
        E1KRXDESC desc, *pDesc = &desc;
        PDMDevHlpPhysRead(pThis->CTX_SUFF(pDevIns), e1kDescAddr(RDBAH, RDBAL, RDH),//从guest中读取一个E1KRXDESC
                          &desc, sizeof(desc));
# endif /* !E1K_WITH_RXD_CACHE */
        if (pDesc->u64BufAddr)//是非空指针
        {
            /* Update descriptor */
            pDesc->status        = status;//设置status
            pDesc->u16Checksum   = checksum;//设置checksum
             ...
             */
            if (cb > pThis->u16RxBSize)//cb大于2048,需要分片
            {
                pDesc->status.fEOP = false;//设置fEOP等于false
                e1kCsRxLeave(pThis);//离开临界区
                e1kStoreRxFragment(pThis, pDesc, ptr, pThis->u16RxBSize);//把一个分片写回guest
                rc = e1kCsRxEnter(pThis, VERR_SEM_BUSY);//进入临界区
                if (RT_UNLIKELY(rc != VINF_SUCCESS))
                    return rc;
                ptr += pThis->u16RxBSize;//更新ptr指针
                cb -= pThis->u16RxBSize;//更新cb
            }
            else//不需要分片
            {
                pDesc->status.fEOP = true;//设置fEOP等于true
                e1kCsRxLeave(pThis);//离开临界区
                e1kStoreRxFragment(pThis, pDesc, ptr, cb);//把包的内容写回guest
# ifdef E1K_WITH_RXD_CACHE
                rc = e1kCsRxEnter(pThis, VERR_SEM_BUSY);//进入临界区
                if (RT_UNLIKELY(rc != VINF_SUCCESS))
                    return rc;
                cb = 0;//包的内容全部都写回guest了,所以设置cb等于0
# else /* !E1K_WITH_RXD_CACHE */
...
# ifdef E1K_WITH_RXD_CACHE
        /* Write back the descriptor. */
        pDesc->status.fDD = true;
        e1kRxDPut(pThis, pDesc);//把pDesc的内容写回到guest
# else /* !E1K_WITH_RXD_CACHE */
        else
        {
            /* Write back the descriptor. */
            pDesc->status.fDD = true;
            PDMDevHlpPCIPhysWrite(pThis->CTX_SUFF(pDevIns),//把pDesc的内容写回到guest
                                  e1kDescAddr(RDBAH, RDBAL, RDH),
                                  pDesc, sizeof(E1KRXDESC));
            e1kAdvanceRDH(pThis);
        }
...
# ifdef E1K_WITH_RXD_CACHE
    /* Complete packet has been stored -- it is time to let the guest know. */
#  ifdef E1K_USE_RX_TIMERS
    if (RDTR)//包已经接收完成,使用timer通知guest
    {
        /* Arm the timer to fire in RDTR usec (discard .024) */
        e1kArmTimer(pThis, pThis->CTX_SUFF(pRIDTimer), RDTR);
        /* If absolute timer delay is enabled and the timer is not running yet, arm it. */
 ...
 */
static int e1kRegReadCTRL(PE1KSTATE pThis, uint32_t offset, uint32_t index, uint32_t *pu32Value)//读取控制寄存器
{
 ...
 * @thread  any
 */
void e1kPhyLinkResetCallback(PPHY pPhy)//重置时的回调函数
{
 ...
 * @thread  EMT
 */
static int e1kRegWriteCTRL(PE1KSTATE pThis, uint32_t offset, uint32_t index, uint32_t value)//写控制寄存器
{
    int rc = VINF_SUCCESS;

    if (value & CTRL_RESET)//value有CTRL_RESET标志
    { /* RST */
#ifndef IN_RING3
        return VINF_IOM_R3_MMIO_WRITE;
#else
        e1kHardReset(pThis);//将e1000设备重置
#endif
    }
    else//value没有CTRL_RESET标志
    {
         ...
         */
        if (   (value & CTRL_SLU)//value有CTRL_SLU标志 且
            && !(CTRL & CTRL_SLU)//CTRL寄存器没有CTRL_SLU标志 且
            && pThis->fCableConnected//fCableConnected是true 且
            && !(STATUS & STATUS_LU))//STATUS不是link up 状态
        {
            /* It should take about 2 seconds for the link to come up */
            e1kArmTimer(pThis, pThis->CTX_SUFF(pLUTimer), E1K_INIT_LINKUP_DELAY_US);//定时器,需要2秒来link up
        }
#else /* !E1K_LSC_ON_SLU */
        if (   (value & CTRL_SLU)//value有CTRL_SLU标志 且
            && !(CTRL & CTRL_SLU)//CTRL没有CTRL_SLU标志 且
            && pThis->fCableConnected//fCableConnected是true 且
            && !TMTimerIsActive(pThis->CTX_SUFF(pLUTimer)))
        {
            /* PXE does not use LSC interrupts, see @bugref{9113}. */
            STATUS |= STATUS_LU;//为STATUS添加SATUS_LU标志
        }
#endif /* !E1K_LSC_ON_SLU */
        if ((value & CTRL_VME) != (CTRL & CTRL_VME))//value和CTRL寄存器不同时具有CTRL_VME标志
        {
        ...
        if (value & CTRL_MDC)//value有CTRL_MDC标志
        {
            if (value & CTRL_MDIO_DIR)//value有CTRL_MDIO_DIR标志
            {
            ...
            else//value没有CTRL_MDIO_DIR标志
            {
                if (Phy::readMDIO(&pThis->phy))//
                    value |= CTRL_MDIO;//为value添加CTRL_MDIO标志
                else
                    value &= ~CTRL_MDIO;//从value中去掉CTRL_MDIO标志
                Log7(("%s e1kRegWriteCTRL: Phy::readMDIO(%d)\n", pThis->szPrf, !!(value & CTRL_MDIO)));
            }
        }
        rc = e1kRegWriteDefault(pThis, offset, index, value);//把value的值写入到寄存器
    }
 ...
 * @thread  EMT
 */
static int e1kRegWriteEECD(PE1KSTATE pThis, uint32_t offset, uint32_t index, uint32_t value)//写EEPROM
{
 ...
 * @thread  EMT
 */
static int e1kRegReadEECD(PE1KSTATE pThis, uint32_t offset, uint32_t index, uint32_t *pu32Value)//读EEPROM
{
#ifdef IN_RING3
    uint32_t value;
